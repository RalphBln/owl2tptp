 ...
 * @author ralph
 */
public class AspectAnnotationOWL2TPTPObjectRenderer extends OWL2TPTPObjectRenderer implements AspectOWLObjectRenderer<Stream<FolFormula>> {
    ...
    public boolean hasAspect(OWLAxiom axiom) {
        return axiom.isAnnotated() && axiom.componentsAnnotationsFirst().filter(c -> c instanceof OWLAnnotation && ((OWLAnnotation) c).getProperty().asOWLAnnotationProperty().getIRI().equals(aspectAnnotationProperty)).findAny().map(c -> true).get();
    }
